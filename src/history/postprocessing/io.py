...
def uncompress_all_submissions(
    ...
    if verbose:
        ...
        if space_info['estimated_extracted_gb'] > space_info['available_gb']:
            print(f"WARNING: Estimated extracted size exceeds available space!")
        else:
...
def analyze_submissions(data_dir: Union[str, Path], verbose: bool = True) -> tuple[pd.DataFrame, Dict[str, Dict[str, str]]]:
    """
    ...
    for submission_dir in data_dir.iterdir():
        ...
        if verbose:
            ...
            if missing_file_types:
                print(f"Warning: {submission_dir.name} is missing mandatory file types: {', '.join(missing_file_types)}")
        ...
        for file_type, files in mandatory_file_matches.items():
            for f in files:
                ...
                if filename.lower().endswith('sparse_pointcloud.laz') or filename.lower().endswith('sparse_pointcloud.las'):
                    prefix = filename.replace('sparse_pointcloud.laz', '').replace('sparse_pointcloud.las', '').replace('sparse_pointcloud.LAZ', '').replace('sparse_pointcloud.LAS', '').rstrip('_')
                elif filename.lower().endswith('dense_pointcloud.laz') or filename.lower().endswith('dense_pointcloud.las'):
                    prefix = filename.replace('dense_pointcloud.laz', '').replace('dense_pointcloud.las', '').replace('dense_pointcloud.LAZ', '').replace('dense_pointcloud.LAS', '').rstrip('_')
            ...
            for author, author_experiments in experiments.items():
                for experiment_code, file_presence in author_experiments.items():
                    ...
                    if len(present_files) > 1:  # Only check if we have multiple files
                        ...
                        if len(prefixes) > 1:
                            print(f"Warning: {submission_dir.name} experiment {experiment_code} has files with different codes: {', '.join(sorted(prefixes))}")
...
def combine_intrinsics_files(files_dict: Dict[str, Dict[str, str]]) -> pd.DataFrame:
    ...
    if combined_data:
        ...
        info_cols = ['experiment_code', 'author', 'site', 'dataset', 'images', 'calibration', 'gcp', 'coregistration', 'multi-temporal']
...
def filter_experiment_data(df: pd.DataFrame, site: str = None, images: str = None, dataset: str = None, 
                          calibration: str = None, gcp: str = None, coregistration: str = None, 
                          multi_temporal: str = None) -> pd.DataFrame:
    """
