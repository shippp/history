...
- reference DEM, seamless mosaic of ArcticDEM strips and lidar DEM over glaciers, provided by Joaquin.
...
# update readme etc
# add roi gpkg for Casa Grande

# Create output folder
outfolder = "./iceland/aux/"
os.makedirs(outfolder, exist_ok=True)

overwrite = False

# Bounding box of the two area of interest, in ISN2016 (EPSG:8088)
# Chosen to be multiple of 30 m (with gdalwarp -tap)
# zoom_bounds = [2604540,  118500, 2768730, 208620]
...
glacier_poly_fn = os.path.join(outfolder, "RGI2000-v7.0-G-06_iceland.zip")
...
lidar_dem_fn = os.path.join(outfolder, "iceland_refdem.tif")
...
if overwrite or (not os.path.exists(lidar_dem_fn)):
    ...
    cmd = f"wget -c -r -np -nd -A '*isn2016*.tif' -P {lidar_folder} https://ftp.lmi.is/.stm/joaquin/history/iceland/refdem_isn2016/"
    ...
    cmd = f"gdalbuildvrt -r nearest {tmp_vrt_fn} -input_file_list {list_fn} -resolution highest"
    print(cmd); subprocess.run(cmd, shell=True, check=True)

    # Crop
    bbox_gdal = " ".join([str(bound) for bound in zoom_bounds])
    cmd = f"gdalwarp {tmp_vrt_fn} {lidar_dem_fn} -te {bbox_gdal} -tr 2 2 -co COMPRESS=DEFLATE -co tiled=yes -co bigtiff=if_safer"
    print(cmd); subprocess.run(cmd, shell=True, check=True)
...
if overwrite or (not os.path.exists(cop30_uncoreg_fn)):
    ...
    # final_cop30_fn = os.path.join(outfolder, "iceland_cop30_dem.tif")
...
cop30_coreg_fn = os.path.join(outfolder, "iceland_cop30_dem.tif")
...
if overwrite or (not os.path.exists(cop30_coreg_fn)):
    ...
    ddem_after.save(os.path.join(outfolder, "iceland_dem-diff.tif"), tiled=True)
    inlier_mask_vert.save(os.path.join(outfolder, "coreg_mask.tif"), tiled=True)
...
mask_large.save(os.path.join(outfolder, "iceland_cop30_dem_mask.tif"), tiled=True)
...
mask_zoom.save(os.path.join(outfolder, "iceland_refdem_mask.tif"), tiled=True)
